...
import { PokedexServiceService } from '../../../servicios/pokedex-service/pokedex-service.service';
import { FavoritoService } from '../../../servicios/favorito-service/favorito.service';

@Component({
  selector: 'app-pokedex',
  standalone: true,
  imports: [PokemonComponent, MayusPipePipe, ReactiveFormsModule, RouterLink],
...
export class PokedexComponent implements OnInit {
  ...
  @Input() pokemon: any;
  @Input() pokemonAnt: any;
  @Input() pokemonSig: any;
    ...
    constructor(private consultaService: ConsultaserviceService, private route: ActivatedRoute, private PokedexServiceService: PokedexServiceService,
      ...
      this.busqueda = new FormGroup({
        nombrePkmn: new FormControl('', Validators.required)
      });

      this.route = inject(ActivatedRoute);
      this.consultaService = inject(ConsultaserviceService);
      // this.route.params.subscribe(params => console.log(params));
      // this. = this.consultaService.buscarPokemon(this.route.snapshot.paramMap.get('id'));
      // this.listaPokemons = this.consultaService.buscarTodosLosPokemon();
      // this.consultaService.buscarPokemon(this.nombrePkmn);
    ...
    ngOnInit():void {
     ...
     getPokemonId(id: number): void {
      this.consultaService.buscarPokemon(id).subscribe({
        next: (data: any) => {
     ...
     loadEntries() {
      this.PokedexServiceService.getEntries().subscribe({
        next: (data: any) => {
          this.pokemonData = data;
          console.log(this.pokemonData);
        },
      ...
      // getDatos() {
      //   this.consultaService.buscarTodosLosPokemon().subscribe({
      //     next: (data: any) => {
      //       this.pokemon = data;
    ...
    //       this.favoritoService.addTofavoritos(this.pokemon).subscribe({
    //         next: (data: any) => {
    //           this.pokemon = data; 
    //           console.log("Pokemon faveado");
    ...
    //     })
    //   }else{
    //     console.log("No seleccionado favorito");
    //   }
    // }


  getPokemon(nombrePkmn: string) {
        this.consultaService.buscarPokemonNombre(nombrePkmn).subscribe({
          next: (data: any) => {
            this.pokemon = data;
            
            this.buscarPokemonAnterior(this.pokemon.id);
            this.buscarPokemonSiguiente(this.pokemon.id);
            // console.log(this.pokemon);
  ...
  buscarPokemonAleatorio() {
    ...
    console.log("Buscando pokemon random ");
   ...
   buscarPokemonAnterior(id: number) {
    
    this.consultaService.buscarPokemon(id - 1).subscribe({
        next: (data: any) => {
          this.esShiny = false;
          this.pokemonAnt = data;
          if(this.pokemonAnt?.id === undefined){
            console.log('No se encontró el pokemon');
          }else{
            // console.log("Numero de pokemon = " + this.pokemonAnt.id);
            ...
            console.log(this.pokemonAnt.name);
    ...
    buscarPokemonSiguiente(id: number) {

      this.consultaService.buscarPokemon(id + 1).subscribe({
        next: (data: any) => {
          
          this.pokemonSig = data;
          if(this.pokemonSig?.id === undefined){
            console.log('No se encontró el pokemon');
          }else{
            ...
            console.log(this.pokemonSig.name);
      ...
      // getIdPokemon(nombrePokemon: string): number {
      //   const entry = this.pokedex[0].entries.find(entry => entry.nombre.toLowerCase() === nombrePokemon.toLowerCase());
      ...
      // cargarPokemons() {
      //   let id = this.consultaService.conseguirIdNombre(this.busqueda.get('nombrePkmn')?.value);
      //   console.log(id);

        // this.consultaService.buscarPokemon(id).subscribe({
        //   next: (data: any) => {
        //     this.consultaService.buscarPokemon(data.id - 1).subscribe(prev => {
        //     this.pokemons[0] = prev;
        //   });
        //   this.consultaService.buscarPokemon(data.id).subscribe(curr => {
        //     this.pokemons[1] = curr;
        //   });
        //   this.consultaService.buscarPokemon(data.id + 1).subscribe(next => {
        //     this.pokemons[2] = next;
        //   })
      ...
      buscarPokemon() {
       this.getPokemon(this.busqueda.get('nombrePkmn')?.value);
      }

      // buscarPokemonAnterior() {
      //   this.getPokemon(this.pokemon.id - 1);
      // }

      // buscarPokemonSiguiente() {
      ...
      // }
        ...
        //   .then(pokedex => {
        //   console.log(pokedex.entries[0].nombre); // Imprime 'Bulbasaur'
        //   });
        // }
        ...
        calcularAltura(altura: number) {
          return parseFloat((altura / 10).toFixed(1));
        }

...
export class PokedexComponent implements OnInit {
  ...
  @Input() pokemon: Pokemon = {
    ...
    ngOnInit() {
      // this.getDatos();
      // this.leerJSON();
      this.pokemon.nPokedex = this.consultaService.buscarPokemon(
        this.route.snapshot.paramMap.get('nPokedex'));
     }

     getDescripcionPokemon(nombrePokemon: string): string {
      const entry = this.pokedex[0].entries.find(entry => entry.nombre.toLowerCase() === nombrePokemon.toLowerCase());
      return entry ? entry.descripcion : 'Descripción no encontrada.';
    }
    ...
    getDatos() {
      ...
      getPokemon(nombrePkmn: string) {
        ...
        this.consultaService.buscarPokemon(this.formdex.get('nombrePkmn')?.value).subscribe({
          next: (data: any) => {
            this.pokemon = data;
            console.log(this.pokemon);
          }, 
